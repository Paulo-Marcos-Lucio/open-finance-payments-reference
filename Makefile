...
help:
	...
	@echo "  it         - mvn verify (unit + integration end-to-end com simulator in-process)"
	@echo "  run        - roda app no profile local (sem mTLS, gateway holder vai contra a URL configurada)"
	@echo "  run-sim    - roda app no profile local + simulator (holder embutido no mesmo Spring context)"
	@echo "  load       - dispara tráfego sintético end-to-end pra alimentar dashboards Grafana"
...
up:
	docker compose up -d
	@echo "Grafana: http://localhost:3000 | Swagger: http://localhost:8082/swagger-ui.html"
